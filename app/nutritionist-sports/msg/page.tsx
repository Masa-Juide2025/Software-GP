...
export default function ProfessionalChatPage() {
  ...
  const loadData = async (isFirstLoad = false) => {
    ...
    if (!email) return;
    const url = `/api/nutritionist/msgPage?email=${email}${selectedUser ? `&otherId=${selectedUser.id}` : ""}`;
  ...
  const handleSend = async () => {
    ...
    const res = await fetch("/api/nutritionist/msgPage", {
      method: "POST",
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#fdfcf9]">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.sender === "professional" ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm relative ${msg.sender === "professional" ? 'bg-[#004d3d] text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none'}`}>
                    {msg.message}
                    <div className={`text-[8px] mt-1 flex items-center justify-end gap-1 ${msg.sender === "professional" ? 'text-gray-300' : 'text-gray-400'}`}>
                       {new Date(msg.createdAt).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
                       {msg.sender === "professional" && <CheckCheck className="h-3 w-3" />}
                    </div>
                  </div>
                </div>
              ))}
