  ...
  useEffect(() => {
    const fetchSubscribers = async () => {
      try {
        ...
        const res = await fetch(`/api/nutritionist/subscribers-weight-gain?email=${userEmail}&t=${Date.now()}`)
  ...
  return (
    <div className="p-6 space-y-6 bg-[#f9fafb] min-h-screen" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 font-sans text-right">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">المشتركين - زيادة الوزن</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2 font-sans">
        {filtered.map((sub) => {
          ...
          // حساب الوزن المثالي (معادلة تقريبية لزيادة الوزن)
          const idealWeight = height > 0 ? parseFloat((50 + 0.9 * (height - 152.4)).toFixed(1)) : 0;
          
          // المتبقي للمثالي (في حالة الزيادة: المثالي - الحالي)
          const remainingToGoal = (idealWeight - currentWeight).toFixed(1);

          // حساب نسبة الإنجاز لزيادة الوزن
          const totalToGain = idealWeight - startWeight;
          const gainedSoFar = currentWeight - startWeight;
          let calculatedPercentage = 0;

          if (totalToGain > 0) {
            // إذا كان الوزن الحالي أكبر من أو يساوي المثالي فهي 100%
            calculatedPercentage = Math.max(0, Math.min(100, Math.round((gainedSoFar / totalToGain) * 100)));
          } else if (currentWeight >= idealWeight && idealWeight > 0) {
                        ...
                        <div className="space-y-3 pt-4 border-t">
                          ...
                          <div className="relative">
                            ...
                            <button 
                              ...
                              className="absolute bottom-3 left-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg px-4 flex items-center gap-2 h-9 transition-colors"
                            >
