...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const res = await fetch(`/api/nutritionist/profiles?email=${userEmail}`)
    ...
    try {
      const res = await fetch(`/api/nutritionist/profiles`, {
        ...
        body: JSON.stringify({
          ...
          password: newPassword // إرسال الباسورد الجديد
        }),
      })
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-4 py-2 border-b">
                <Avatar className="h-20 w-20 border-2 border-primary/10">
                  <AvatarFallback className="bg-primary/10 text-xl text-primary font-bold">
                    {profile.fullName ? profile.fullName.substring(0, 2) : "أخ"}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="text-lg font-bold text-primary">{profile.fullName}</h3>
                  <p className="text-sm text-muted-foreground">{"أخصائي تغذية"}</p>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label>{"الحد الأقصى للمشتركين"}</Label>
                  <Input 
                    type="number" 
                    value={profile.maxSubscribers} 
                    onChange={(e) => setProfile({...profile, maxSubscribers: Number(e.target.value)})}
                  />
                </div>
              </div>
