...
export default function ProfessionalChatPage() {
  ...
  const loadData = async (isFirstLoad = false) => {
    ...
    if (!email) return;
    const url = `/api/trainer/msgPage?email=${email}${selectedUser ? `&otherId=${selectedUser.id}` : ""}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.success) {
      if (isFirstLoad) setContacts(data.contacts);
      // هون الروت برجع trainerId، رح نخزنه في الـ state
      setProfessionalId(data.trainerId); 
  ...
  const handleSend = async () => {
    if (!newMessage.trim() || !selectedUser || !professionalId) return;

    // التعديل هون: غيرنا professionalId لـ trainerId عشان يطابق الـ POST بالروت
    const res = await fetch("/api/trainer/msgPage", {
      method: "POST",
    ...
    <div className="flex h-[85vh] gap-4 bg-[#f8fafc] p-2" dir="rtl">
      ...
      <div className="w-1/3 flex flex-col gap-4">
        <div className="relative pt-2">
          ...
          <Input placeholder="بحث في المشتركين..." className="pr-10 bg-white border-none shadow-sm h-11 rounded-xl text-right" />
        </div>
            ...
            <Card key={contact.id} className={`cursor-pointer border-none shadow-sm transition-all ${selectedUser?.id === contact.id ? 'bg-[#004d3d] text-white' : 'bg-white hover:bg-gray-50'}`} onClick={() => setSelectedUser(contact)}>
              <CardContent className="p-4 flex items-center gap-3 text-right">
                ...
                <div className="flex-1">
                  ...
                  <div className={`text-[10px] ${selectedUser?.id === contact.id ? 'text-gray-200' : 'text-muted-foreground'}`}>مشترك نشط</div>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Card className="flex-1 flex flex-col bg-white border-none shadow-sm overflow-hidden rounded-2xl">
        {selectedUser ? (
          <>
            <div className="p-4 border-b flex items-center justify-between sticky top-0 bg-white z-10">
              <div className="flex items-center gap-3 text-right">
                ...
                <div>
                  ...
                  <div className="text-[9px] text-green-500 font-bold">مشترك مخصص لك</div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#fdfcf9]">
              {messages.map((msg, idx) => (
                // التعديل هون: المدرب هو الـ professional (أو trainer) في هذه الصفحة
                <div key={idx} className={`flex ${msg.sender === "trainer" ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm relative ${msg.sender === "trainer" ? 'bg-[#004d3d] text-white rounded-br-none' : 'bg-white text-gray-800 border rounded-bl-none'}`}>
                    {msg.message}
                    <div className={`text-[8px] mt-1 flex items-center justify-end gap-1 ${msg.sender === "trainer" ? 'text-gray-300' : 'text-gray-400'}`}>
                       {new Date(msg.createdAt).toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}
                       {msg.sender === "trainer" && <CheckCheck className="h-3 w-3" />}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 bg-white border-t flex gap-2">
              <Input placeholder="اكتب ردك هنا..." className="flex-1 bg-gray-50 border-none h-11 rounded-xl text-right" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} />
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground gap-3">
             ...
             <p className="font-bold text-[#004d3d] opacity-60">اختر مشتركاً للرد على استفساراته</p>
          </div>
        )}
      </Card>
    </div>
