...
import { Star, Eye, X, Mail, Phone, CalendarDays, Users, Dumbbell, Search } from "lucide-react";
...
export function TrainersGrid() {
  ...
  useEffect(() => {
    // جلب البيانات من الـ API الخاص بالمدربين
  ...
  // دالة تصفية المدربين بناءً على البحث
  const filteredTrainers = trainers.filter((trainer: any) => {
    const query = searchQuery.toLowerCase();
    return (
      trainer.displayName?.toLowerCase().includes(query) ||
  ...
  return (
    <>
      {/* --- خانة البحث بنفس ستايل الأخصائيين والمشتركين --- */}
      <div className="relative mb-10 max-w-md" dir="rtl">
        <div className="relative">
          <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
          <input
            type="text"
            placeholder="ابحث عن مدرب بالاسم، الرقم، أو التخصص..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8" dir="rtl">
        {filteredTrainers.length > 0 ? (
          filteredTrainers.map((trainer: any) => (
            <div key={trainer._id} className="bg-white rounded-[35px] p-8 shadow-sm border border-gray-100 hover:shadow-md transition-all relative overflow-hidden">
              
              {/* الجزء العلوي: الاسم والتخصص */}
              <div className="flex justify-between items-start mb-6">
                <div className="flex gap-5 text-right">
                  <div className="w-16 h-16 rounded-full bg-[#ecfdf5] text-[#107c41] flex items-center justify-center font-black text-2xl font-sans">
                    {trainer.displayName?.substring(0, 2).toUpperCase() || "TR"}
                  </div>
                  <div>
                    <h3 className="font-black text-gray-800 text-xl mb-1">
                      {trainer.displayName || "اسم غير معروف"}
                    </h3>
                ...
                <span className="bg-[#107c41] text-white px-4 py-1.5 rounded-xl text-xs font-black flex items-center gap-1">
                  <Dumbbell size={14}/>
                  {trainer.specialization || "تدريب"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-8 mb-6 font-sans">
                <div className="flex items-center gap-1.5 text-yellow-500">
                  ...
                  <span className="text-gray-800 font-black">{trainer.rating || "4.8"}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-400 font-black text-sm">
                  <span>{trainer.experienceYears || "0"} سنوات</span>
                </div>
                <div className="flex items-center gap-2 text-gray-400 font-black text-sm">
                   <span>{trainer.currentClients || "0"} مشترك</span>
                </div>
              </div>
          ...
          <div className="col-span-full py-20 text-center bg-gray-50 rounded-[35px] border-2 border-dashed border-gray-200">
            <p className="font-black text-gray-400 text-lg">لا يوجد مدربين يطابقون بحثك!</p>
          </div>
        )}
      </div>

      {/* الـ Modal - نافذة التفاصيل */}
      {selectedTrainer && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedTrainer(null)} dir="rtl">
          <div className="bg-white rounded-[30px] p-8 w-full max-w-2xl relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            
            {/* زر الإغلاق */}
            <button onClick={() => setSelectedTrainer(null)} className="absolute top-6 left-6 text-gray-400 hover:text-gray-600">
              <X size={24} />
            </button>
            ...
            <div className="flex flex-col items-center text-center mb-8">
              <div className="w-20 h-20 rounded-full bg-[#ecfdf5] text-[#107c41] flex items-center justify-center font-black text-3xl font-sans mb-4">
                {selectedTrainer.displayName?.substring(0, 2).toUpperCase()}
              </div>
              <h2 className="text-2xl font-black text-gray-800 mb-1">{selectedTrainer.displayName}</h2>
              <p className="text-gray-400 font-black text-sm mb-3">
                {selectedTrainer.specialization} - مدرب رياضي
              </p>
            ...
            <div className="border-t border-gray-100 pt-6 mb-8 text-right">
              <h4 className="font-black text-gray-800 mb-4">معلومات التواصل</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-600">
                <div className="flex items-center gap-2">
                  <Mail size={18} className="text-[#107c41]" />
                  <span className="font-bold">{selectedTrainer.email || "غير متوفر"}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Phone size={18} className="text-[#107c41]" />
                  <span className="font-bold">{selectedTrainer.phone || "غير متوفر"}</span>
                </div>
                <div className="flex items-center gap-2 col-span-full">
                  <CalendarDays size={18} className="text-[#107c41]" />
                  <span className="font-bold text-gray-800">أيام العمل: </span>
            ...
            <div className="border-t border-gray-100 pt-6 mb-8 text-right">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="font-black text-gray-800">قائمة المشتركين ({selectedTrainer.currentClients})</h4>
              </div>
              
              <div className="space-y-3">
                {selectedTrainer.subscriberList && selectedTrainer.subscriberList.length > 0 
                  ? selectedTrainer.subscriberList.map((sub: any, index: number) => (
                  ...
                  : <p className="text-gray-400 text-sm p-4 text-center bg-gray-50 rounded-2xl font-black">لا يوجد مشتركين حالياً</p>
                }
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center border-t border-gray-100 pt-6">
              <div className="bg-gray-50 p-4 rounded-2xl">
                <div className="text-2xl font-black text-gray-800">{selectedTrainer.maxClients || "0"}</div>
                <div className="text-xs text-gray-500 font-black">الحد الأقصى</div>
              </div>
              <div className="bg-gray-50 p-4 rounded-2xl">
                <div className="text-2xl font-black text-gray-800">{selectedTrainer.currentClients || "0"}</div>
                <div className="text-xs text-gray-500 font-black">مشترك نشط</div>
              </div>
              <div className="bg-gray-50 p-4 rounded-2xl">
                <div className="text-2xl font-black text-gray-800">{selectedTrainer.experienceYears || "0"}</div>
